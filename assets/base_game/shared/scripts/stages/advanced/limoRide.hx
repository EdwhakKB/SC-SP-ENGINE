import objects.stageobjects.BackgroundDancer;

// Limo
var fastCarCanDrive:Bool = true;
var limoSpeed:Float = 0;
var grpLimoParticles:FlxSpriteGroup;
...
// event
var limoKillingState:HenchmenKillState = HenchmenKillState.WAIT;
...
function onCreate()
{
  ...
  stageSpriteHandler(skyBG, -1, 'skyBG');

  limoMetalPole = new BGSprite('gore/metalPole', -500, 220, 0.4, 0.4);
  if (!ClientPrefs.data.lowQuality) stageSpriteHandler(limoMetalPole, -1, 'limoMetalPole');

  bgLimo = new BGSprite('limo/bgLimo', -150, 480, 0.4, 0.4, ['background limo pink'], true);
  if (!ClientPrefs.data.lowQuality) stageSpriteHandler(bgLimo, -1, 'bgLimo');

  limoCorpse = new BGSprite('gore/noooooo', -500, limoMetalPole.y - 130, 0.4, 0.4, ['Henchmen on rail'], true);
  if (!ClientPrefs.data.lowQuality) stageSpriteHandler(limoCorpse, -1, 'limoCorpse');

  limoCorpseTwo = new BGSprite('gore/noooooo', -500, limoMetalPole.y, 0.4, 0.4, ['henchmen death'], true);
  if (!ClientPrefs.data.lowQuality) stageSpriteHandler(limoCorpseTwo, -1, 'limoCorpseTwo');

  grpLimoDancers = new FlxSpriteGroup();
  if (!ClientPrefs.data.lowQuality) stageSpriteHandler(grpLimoDancers, -1, 'grpLimoDancers');
  ...
  if (!ClientPrefs.data.lowQuality) stageSpriteHandler(limoLight, -1, 'limoLight');

  grpLimoParticles = new FlxSpriteGroup();
  if (!ClientPrefs.data.lowQuality) stageSpriteHandler(grpLimoParticles, -1, 'grpLimoParticles');
  ...
  fastCar.active = true;
  stageSpriteHandler(fastCar, 4, 'fastCar');

  var limo:BGSprite = new BGSprite('limo/limoDrive', -120, 550, 1, 1, ['Limo stage'], true);
  stageSpriteHandler(limo, 0, 'limo'); // Shitty layering but whatev it works LOL
...
function onUpdate(elapsed)
{
  ...
  {
    if (grpLimoParticles != null) grpLimoParticles.forEach(function(spr:BGSprite) {
          ...
          if (dancers[i].x < FlxG.width * 1.5 && limoLight.x > (370 * i) + 170)
          {
            switch (i)
            {
              case 0 | 3:
                ...
                var particle:BGSprite = new BGSprite('gore/noooooo', dancers[i].x + 160, dancers[i].y + 200, 0.4, 0.4, ['hench arm spin' + diffStr + 'PINK'],
                  false);
                grpLimoParticles.add(particle);
