...
import backend.song.data.SongData.SongMusicData;
...
class FunkinSound extends FlxSound implements ICloneable<FunkinSound>
{
  ...
  override function onFocus():Void
  {
    ...
    {
      Debug.logInfo('Resuming audio (${this._label}) on focus!');
      resume();
    }
    else
    {
      Debug.logInfo('Not resuming audio (${this._label}) on focus!');
    }
    _lostFocus = false;
  }
  ...
  {
    Debug.logInfo('Focus lost, pausing audio!');
    _lostFocus = true;
  ...
  public static function playMusic(key:String, params:FunkinSoundPlayMusicParams):Bool
  {
    ...
    if (!(params.restartTrack ?? false) && FlxG.sound.music?.playing)
    {
      if (FlxG.sound.music != null && Std.isOfType(FlxG.sound.music, FunkinSound))
      {
        ...
        if (existingSound._label == Paths.getPath('music/$key/$key.ogg', MUSIC))
        {
          return false;
        }
      }
    ...
    {
      case MUSIC: Paths.getPath('music/$key/$key.ogg', MUSIC);
      case INST: Paths.getInstPath(key, [prefix, suffix, true]);
      default: Paths.getPath('music/$key/$key.ogg', MUSIC);
    }
      ...
      ?onComplete:Void->Void, ?onLoad:Void->Void):Null<FunkinSound>
  {
    ...
    {
      Debug.logError('FunkinSound could not play sound, channels exhausted! Found ${SoundMixer.__soundChannels.length} active sound channels.');
      return null;
    }
   ...
   * @return A `FunkinSound` object, or `null` if the sound could not be loaded.
   */
  public static function loadASound(embeddedSound:FlxSoundAsset, volume:Float = 1.0, looped:Bool = false, autoDestroy:Bool = false, autoPlay:Bool = false,
      ?onComplete:Void->Void, ?onLoad:Void->Void):Null<FunkinSound>
  {
    @:privateAccess
    if (SoundMixer.__soundChannels.length >= SoundMixer.MAX_ACTIVE_CHANNELS)
    ...
    var sound:FunkinSound = pool.recycle(constructASound);
    sound.loadEmbedded(embeddedSound, looped, autoDestroy, onComplete);
  ...
  static function constructASound():FunkinSound
  {
    var sound:FunkinSound = new FunkinSound();
    pool.add(sound);
    return sound;
...
typedef FunkinSoundPlayMusicParams =
{
  ...
  /**
   * The prefix of the music file to play. Not usually for tracks when loading an INST file
   * @default ``
   */
  var ?prefix:String;
