...
{
  /**
   * Checks that a given version number satisisfies a given version rule.
  ...
  public static function validateVersion(version:thx.semver.Version, versionRule:thx.semver.VersionRule):Bool
  {
    ...
    {
      Debug.logInfo('[VERSIONUTIL] Invalid semantic version: ${version}');
      return false;
    }
  }
  ...
  public static function repairVersion(version:thx.semver.Version):thx.semver.Version
  {
    ...
    if (thx.Types.isAnonymousObject(versionData.version))
    {
      ...
      Debug.logInfo('[SAVE] Version data repair required! (got ${versionData.version})');
   ...
   */
  public static function getVersionFromJSON(input:Null<String>, isMetaData:Bool = false):Null<thx.semver.Version>
  {
    if (input == null) return null;
    var parsed:Dynamic = SerializerUtil.fromJSON(input);
    if (parsed == null) return null;
    if ((!isMetaData && parsed.version == null) || (isMetaData && parsed.songData.playData.version == null)) return null;
    var versionStr:String = isMetaData ? parsed.songData.playData.version : parsed.version; // Dynamic -> String cast
