...
class Note extends FunkinSCSprite implements ICloneable<Note>
{
  ...
  public static var noteTypeList:Array<String> = // Used for backwards compatibility with 0.1 - 0.3.2 charts, though, you should add your hardcoded custom note types here too.
    ['', 'Alt Animation', 'Hey!', 'Hurt Note', 'GF Sing', 'Mom Sing', 'No Animation'];
  ...
  private function set_noteType(value:String):String
  {
    ...
    if (PlayState.currentChart != null
      && PlayState.currentChart.options.splashSkin != "") skin = PlayState.currentChart.options.splashSkin;
  ...
  public function reloadNote(noteStyle:String = '', postfix:String = '')
  {
    ...
    if (isSustainNote)
    {
      ...
      if (changedSkin)
      {
        if (wasPixelNote && !becomePixelNote) // fixes the scaling
        {
          if (PlayState.currentChart != null && !PlayState.currentChart.options.notITG)
          {
            scale.y /= PlayState.daPixelZoom;
            scale.y *= 0.7;
          }
  ...
  override function update(elapsed:Float)
  {
    ...
    if (mustPress)
    {
      canBeHit = (strumTime > Conductor.instance.songPosition - (Conductor.instance.safeZoneOffset * lateHitMult)
        && strumTime < Conductor.instance.songPosition + (Conductor.instance.safeZoneOffset * earlyHitMult));

      if (strumTime < Conductor.instance.songPosition - Conductor.instance.safeZoneOffset && !wasGoodHit) tooLate = true;
    }
    else
    {
      canBeHit = false;

      if (strumTime < Conductor.instance.songPosition + (Conductor.instance.safeZoneOffset * earlyHitMult))
      {
        if ((isSustainNote && prevNote.wasGoodHit) || strumTime <= Conductor.instance.songPosition) wasGoodHit = true;
      }
    }
