...
class NoteSplash extends FunkinSCSprite
{
  ...
  public function new(?splash:String, ?opponentSplashes:Bool = false)
  {
    ...
    if (splash == null)
    {
      if (!opponentSplashes)
      {
        ...
        if (firstPath) splash = "noteSplashes-" + styleChoice;
        else if (secondPath) splash = "notes/noteSplashes-" + styleChoice;
        else
        {
          if (PlayState.SONG != null)
          {
            if (PlayState.SONG.options.splashSkin != null
              ...
              splash = PlayState.SONG.options.disableSplashRGB ? 'noteSplashes' : DEFAULT_SKIN + getSplashSkinPostfix();
          }
        }
      }
      else
        ...
        else
        {
          if (PlayState.SONG != null)
          {
            if (PlayState.SONG.options.splashSkin != null
              ...
              skin = PlayState.SONG.options.disableSplashRGB ? 'noteSplashes' : DEFAULT_SKIN + getSplashSkinPostfix();
          }
        }
      }
    }
  ...
  {
    var path:String = (!skin.contains('notes') && !skin.contains('noteSplashes/')) ? 'images/noteSplashes/$skin.json' : 'images/$skin.json';
