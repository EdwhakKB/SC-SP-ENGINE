...
<<<<<<< Updated upstream
	...
	public function loadStageJson(stage:String, ?stageChanged:Bool = false)
	{
		...
		if (stageData.ratingOffsets != null)
		{
			...
			stageRatingOffsetXOpponent = stageData.ratingOffsets[1][1];
		}
...
=======
  ...
  {
    if (!ClientPrefs.data.background) return;
    songLowercase = songName.toLowerCase();
  ...
  public function loadStage(boyfriend:Character, mom:Character, dad:Character, gf:Character):Void
  {
    if (!ClientPrefs.data.background) return;
    for (i in toAdd)
      add(i);
      ...
      switch (index)
      {
        case 0:
          ...
          for (bg in array)
            add(bg);
        case 1:
          if (ClientPrefs.data.characters) if (dad != null) add(dad);
          for (bg in array)
            add(bg);
        case 2:
          ...
          for (bg in array)
            add(bg);
        case 3:
          if (ClientPrefs.data.characters) if (boyfriend != null) add(boyfriend);
          for (bg in array)
            add(bg);
        case 4:
  ...
  public function loadStageJson(stage:String, ?stageChanged:Bool = false)
  {
    ...
    {
      if (stageData.isPixelStage == true) PlayState.stageUI = "pixel";
    }
  ...
  public function eventCalled(eventName:String, eventParams:Array<String>, strumTme:Float):Void
  {
  ...
  public function eventPushed(event:SongEventData) {}

  // Events
  public function eventPushedUnique(event:SongEventData) {}

  public function setDefaultGF(name:String) // Fix for the Chart Editor on Base Game stages
  {
    var gfVersion:String = PlayState.currentChart.characters.girlfriend;
    if (gfVersion == null || gfVersion.length < 1)
    {
      gfVersion = name;
      PlayState.currentChart.characters.girlfriend = gfVersion;
    }
  }
  ...
  public function initHScript(file:String)
  {
    try
    {
      ...
      if (newScript.parsingExceptions != null && newScript.parsingExceptions.length > 0)
      {
        @:privateAccess
        for (e in newScript.parsingExceptions)
          if (e != null) PlayState.instance.addTextToDebug('ERROR ON LOADING ($file): ${e.message.substr(0, e.message.indexOf('\n'))}', FlxColor.RED);
        newScript.destroy();
        return;
      }
      #else
