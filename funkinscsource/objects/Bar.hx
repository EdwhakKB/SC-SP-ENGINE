...
class Bar extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, image:String = 'healthBar', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1, overlayName:String = "healthBarOverlay")
	{
		super(x, y);
	...
	public function updateBar()
	{
		...
		overlaySprite.setPosition(bg.x, bg.y);
		...
		overlaySprite.clipRect.x = barOffset.x;
		overlaySprite.clipRect.y = barOffset.y;
	...
	public function regenerateClips()
	{
		...
		{
			overlaySprite.setGraphicSize(Std.int(bg.width), Std.int(bg.height));
			overlaySprite.updateHitbox();
			overlaySprite.clipRect = new FlxRect(0, 0, Std.int(bg.width), Std.int(bg.height));
		}
		updateBar();
	}
