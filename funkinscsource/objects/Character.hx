...
class Character extends FunkinSCSprite
{
  ...
  public function loadCharacterFile(json:Dynamic)
  {
    ...
    if (PlayState.currentChart != null) noteSkin = (json.noteSkin != null ? json.noteSkin : PlayState.currentChart.options.arrowSkin);
    else
  ...
  override function update(elapsed:Float)
  {
    ...
    {
      case 'pico-speaker':
        if (animationNotes.length > 0 && Conductor.instance.songPosition > animationNotes[0][0])
        {
          var noteData:Int = 1;
    ...
    if ((flipMode && isPlayer) || (!flipMode && !isPlayer))
    {
      ...
      {
        if (holdTimer >= Conductor.instance.stepCrochet * singDuration * (0.001 #if FLX_PITCH / (FlxG.sound.music != null ? FlxG.sound.music.pitch : 1) #end))
        {
          dance();
          holdTimer = 0;
        }
  ...
  public function allowHoldTimer():Bool
  {
    return !this.isAnimationNull()
      && this.holdTimer > Conductor.instance.stepCrochet * this.singDuration * (0.001 #if FLX_PITCH / FlxG.sound.music.pitch #end)
