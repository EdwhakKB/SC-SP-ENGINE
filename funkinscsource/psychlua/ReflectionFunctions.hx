...
class ReflectionFunctions
{
	...
	public static function implement(funk:FunkinLua)
	{
		...
		funk.set("getPropertyFromGroup", function(obj:String, index:Int, variable:Dynamic, ?allowMaps:Bool = false) {
			...
			if(split.length > 1)
				realObject = LuaUtils.getPropertyLoop(split, true, allowMaps);
			else
	...
	static function checkForOldClassVars(classVar:String){
		switch(classVar){
			case "StrumNote" | "StrumArrow": classVar = "objects.StrumArrow";
			...
			#if LUA_ALLOWED case "FunkinLua": classVar = "psychlua.FunkinLua"; #end
