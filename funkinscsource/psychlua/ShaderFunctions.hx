...
import codenameengine.shaders.CustomCodeShader;
	...
	public static function implement(funk:FunkinLua)
	{
		...
		if (ClientPrefs.data.shaders){
			...
			funk.set("setActorCustomShader", function(id:String, actor:String){
				...
				if (LuaUtils.getObjectDirectly(actor) != null)
					LuaUtils.getObjectDirectly(actor).shader = funnyCustomShader;
			});
			...
			funk.set("setActorNoCustomShader", function(actor:String){
				...
				if (LuaUtils.getObjectDirectly(actor) != null)
					LuaUtils.getObjectDirectly(actor).shader = null;
			});
			...
			funk.set("doTweenShaderFloat", function(tag:String, object:String, floatName:String, newFloat:Float, duration:Float, ease:String, ?swagShader:String = ""){
				#if (!flash && MODS_ALLOWED && sys)
				var tag = tag;
				...
				if(tag != null)
				{
					...
					tag = LuaUtils.formatVariable('tween_$tag');
					var variables = MusicBeatState.getVariables();
					variables.set(tag, FlxTween.num(leObj.getFloat(floatName), newFloat, duration, {ease: LuaUtils.getTweenEaseByString(ease),
						onComplete: function(twn:FlxTween)
						{
							variables.remove(tag);
					...
					FlxTween.num(leObj.getFloat(floatName), newFloat, duration, {ease: LuaUtils.getTweenEaseByString(ease)}, 
						function(num){
							leObj.setFloat(floatName, num); 
						}
					);
