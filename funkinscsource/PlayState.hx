...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (ClientPrefs.downScroll) kadeEngineWatermark.y = FlxG.height - 760;
		if (ClientPrefs.hudStyle == 'GLOW_KADE') add(kadeEngineWatermark);
		...
		add(healthBarBG);
		add(healthBar);
		if (ClientPrefs.hudStyle == 'GLOW_KADE') add(healthBarOverlay);
		add(iconP1);
		add(iconP2);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					// if ()
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false) && !ClientPrefs.getGameplaySetting('notITGMod', true)) {
