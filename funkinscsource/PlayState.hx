...
class PlayState extends MusicBeatState
{
	...
	public var stagePixelShitPart1:String = '';
	public var stagePixelShitPart2:String = '';
	...
	override public function create()
	{
		...
		// Ratings
		ratingsData.push(new Rating('swag')); // default rating

		var rating:Rating = new Rating('sick');
		rating.ratingMod = 1;
		rating.score = 350;
		rating.noteSplash = true;
		rating.healthBouns = 0.02375 * healthGain;
		...
		else
		{
			...
			opponentMode = ClientPrefs.getGameplaySetting('opponent', false);
		...
		curStage = SONG.stage;
		// Debug.logInfo('stage is: ' + curStage);
		...
		containsAPixelTextureForNotes = isPixelStage;
		...
		{
			stagePixelShitPart1 = stageData.ratingSkin[0];
			stagePixelShitPart2 = stageData.ratingSkin[1];
		}
		...
		gf.opponentMode = opponentMode;
		startCharacterPos(gf);
		gf.scrollFactor.set(0.95, 0.95);
		gfGroup.add(gf);
		startCharacterLua(gf.curCharacter);

		var picoSpeakerAllowed = (gfVersion == 'pico-speaker' && !hideGirlfriend);

		if (hideGirlfriend)
		{
			gf.alpha = 0;
			if (gfVersion == 'pico-speaker')
				picoSpeakerAllowed = false;
		}
		...
		dad.opponentMode = opponentMode;
		startCharacterPos(dad, true);
		dadGroup.add(dad);
		startCharacterLua(dad.curCharacter);

		boyfriend = new Boyfriend(0, 0, SONG.player1);
		boyfriend.opponentMode = opponentMode;
		...
		if (showTime)
		{
			...
			{
				timeBar.createGradientBar([FlxColor.BLACK], [boyfriend.iconColor, dad.iconColor]);
			}
			else if (ClientPrefs.colorBarType == 'Reversed Colors')
			{
				timeBar.createGradientBar([FlxColor.BLACK], [dad.iconColor, boyfriend.iconColor]);
			}
		}
		...
		var splashCPU:NoteSplash = new NoteSplash(100, 100, 0);
		...
		if (storyDifficulty != 1)
		{
			...
			if (diffStr != null && diffStr.length > 0)
				...
				diffInfo = " - " + CoolUtil.difficultyFromInt(storyDifficulty);
		}
		...
		if (ClientPrefs.downScroll)
			kadeEngineWatermark.y = FlxG.height - 700;
		if (ClientPrefs.hudStyle == 'GLOW_KADE')
		{
			if (allowTxtColorChanges)
				kadeEngineWatermark.color = dad.iconColor;
		...
		if (allowTxtColorChanges)
			judgementCounter.color = dad.iconColor;
		...
		if (allowTxtColorChanges)
			botplayTxt.color = dad.iconColor;
		add(botplayTxt);
		if (ClientPrefs.downScroll)
		{
		...
		if (storyDifficulty != 1)
		{
			...
			if (diffStr != null && diffStr.length > 0)
				...
				diffSong = ' - ' + CoolUtil.difficultyFromInt(storyDifficulty);
		}
		songInfo = Main.appName + ' - Song Playing: ${SONG.song}' + diffSong;
		...
		{
			// Debug.logInfo('Key $key is type $type');
			switch (type)
			{
				case 'image':
					Paths.image(key);
	...
	public function startSongStuff()
	{
		startCountdown();

		callOnLuas('songIntroStuff', [isStoryMode, seenCutscene]);
	}
	...
	function set_playbackRate(value:Float):Float
	{
		...
		Debug.logInfo('Anim speed: ' + FlxAnimationController.globalSpeed);
	...
	{
		if (ClientPrefs.healthColor)
		{
			healthBar.createGradientBar([boyfriend.iconColor, dad.iconColor], [boyfriend.iconColor, dad.iconColor]);
		}
		else
		{
			healthBar.createGradientBar([0xFF66FF33, 0xFFFF0000], [0xFF66FF33, 0xFFFF0000]);
		}
		healthBar.updateBar();
		...
		if (ClientPrefs.colorBarType == 'Main Colors')
		{
			timeBar.createGradientBar([FlxColor.BLACK], [boyfriend.iconColor, dad.iconColor]);
		}
		else if (ClientPrefs.colorBarType == 'Reversed Colors')
	...
	public function addCharacterToList(newCharacter:String, type:Int)
	{
		switch (type)
		{
			case 0:
				...
				{
					boyfriend.resetAnimationVars();
					var newBoyfriend:Boyfriend = new Boyfriend(0, 0, newCharacter);
					newBoyfriend.resetAnimationVars();
			...
			case 1:
				...
				{
					dad.resetAnimationVars();
					var newDad:Character = new Character(0, 0, newCharacter);
					newDad.resetAnimationVars();
			...
			case 2:
				...
				{
					gf.resetAnimationVars();
					var newGf:Character = new Character(0, 0, newCharacter);
					newGf.resetAnimationVars();
	...
	public function getLuaObject(tag:String, text:Bool = true):FlxSprite
	{
		...
		if (modchartIcons.exists(tag))
			return modchartIcons.get(tag);
	...
	public var stageIntroAssets:Array<String> = ['ready', 'set', 'go'];
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				if (boyfriend != null
					&& idleToBeat
					&& tmr.loopsLeft % boyfriend.danceEveryNumBeats == 0
				...
				if (dad != null
					&& idleToBeat
					&& tmr.loopsLeft % dad.danceEveryNumBeats == 0
				...
				for (value in modchartCharacters.keys())
				{
					...
					if (daChar != null
						...
						&& tmr.loopsLeft % daChar.danceEveryNumBeats == 0
				...
				if (stageIntroAssets.contains('-pixel'))
				{
					antialias = false;
				}
				...
				switch (swagCounter)
				{
					case 0:
						if (has3rdIntroAsset)
						{
							...
							getready.scale.set(0.7, 0.7);
							getready.cameras = [camHUD];
							getready.updateHitbox();

							if (PlayState.isPixelStage)
								getready.setGraphicSize(Std.int(getready.width * daPixelZoom));
							// nothing else shold be added
				...
				notes.forEachAlive(function(note:Note)
				{
					if (ClientPrefs.opponentStrums || note.mustPress)
					{
						...
						if ((ClientPrefs.middleScroll && !note.mustPress && !opponentMode)
							|| (ClientPrefs.middleScroll && note.mustPress && opponentMode))
						{
							note.alpha *= 0.35;
						}
					}
	...
	public function updateScore(miss:Bool = false)
	{
		...
		else if (ClientPrefs.hudStyle == 'GLOW_KADE')
		{
			scoreTxt.text = 'Score: '
				+ songScore
				+ ' • Combo Breakes: '
	...
	private function generateSong():Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				// old method seems to work better for psych engine rather then the new one for kade engine
				var gottaHitNote:Bool = section.mustHitSection;

				if (songNotes[1] > (Note.ammo[mania] - 1) && !opponentMode)
					gottaHitNote = !section.mustHitSection;
				else if (songNotes[1] <= (Note.ammo[mania] - 1) && opponentMode)
					gottaHitNote = !section.mustHitSection;
		...
		// Debug.logInfo(unspawnNotes.length);
		// playerCounter += 1;
	...
	public function generateTimeBar()
	{
		var showTime:Bool = (ClientPrefs.timeBarType != 'Disabled');
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 19, 400, "", 32);
		timeTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		for (i in 0...Note.ammo[mania])
		{
			...
			if (player < 1)
			{
				if (!ClientPrefs.opponentStrums)
					targetAlpha = 0;
				else if (ClientPrefs.middleScroll && !opponentMode)
					targetAlpha = 0.35;
			}

			if (player > 0 && opponentMode)
			{
				if (ClientPrefs.middleScroll && opponentMode)
					targetAlpha = 0.35;
			}

			STRUM_X_MIDDLESCROLL = (opponentMode ? 278 : -278);
	...
	override public function update(elapsed:Float)
	{
		...
		if (unspawnNotes.length > 0)
		{
			...
			while (unspawnNotes[0] != null && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
		...
		iconP1.x = healthBar.x + (healthBar.width * (healthPercent * 0.01)) + (150 * iconP1.scale.x - 150) / 2 - iconOffset;
		iconP2.x = healthBar.x + (healthBar.width * (healthPercent * 0.01)) - (150 * iconP2.scale.x) / 2 - iconOffset * 2;
		...
		else if ((isHealthBarPercentLessThan20 && !opponentMode && iconP2.hasWinning)
			|| (isHealthBarPercentLessThan20 && opponentMode && iconP1.hasWinning))
		{
			if (!opponentMode)
				iconP2.animation.curAnim.curFrame = 2;
			else
		...
		try
		{
			if (generatedMusic && !endingSong && !isCameraOnForcedPos)
			{
				...
				if (dad != null && !camMustHit && cameraTargeted == 'dad')
				{
					charCam = dad;
					isDadCam = true;
				...
				if (gf != null && (camMustHit || !camMustHit) && cameraTargeted == 'gf')
				{
					charCam = gf;
					isGfCam = true;
					...
					camFollow.set(gf.getMidpoint().x + offsetX, gf.getMidpoint().y + offsetY);
				...
				if (boyfriend != null && camMustHit && cameraTargeted == 'bf')
				{
					charCam = boyfriend;
					isDadCam = false;
				...
				if (charCam.animOffsets.exists('singLEFT')
					&& charCam.animOffsets.exists('singDOWN')
					&& charCam.animOffsets.exists('singUP')
					&& charCam.animOffsets.exists('singRIGHT'))
				{
					moveCameraXY(charCam, false, isDadCam, isGfCam);
				}

				callOnLuas('onMoveCamera', [cameraTargeted]);
			}
		}
		catch (e)
		{
		...
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				if (ClientPrefs.instantRespawn
					|| !char.animOffsets.exists('firstDeath')
					&& !char.animOffsets.exists('deathLoop')
					&& !char.animOffsets.exists('deathConfrim'))
				{
					CustomFadeTransition.nextCamera = mainCam;
				...
				{
					openSubState(new GameOverSubstate(char.getScreenPosition().x - char.positionArray[0], char.getScreenPosition().y - char.positionArray[1],
	...
	public function getControl(key:String)
	{
		...
		// Debug.logInfo('Control result: ' + pressed);
		return pressed;
	}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			...
			case 'Play Animation':
				// Debug.logInfo('Anim to play: ' + value1);
	...
	public function changeBoyfriendCharacter(char:String, charType:Int, ?spriteAllowedXY:Bool = false, ?x:Float, ?y:Float)
	{
		...
		ResetAnimationVars(boyfriend, false);
	...
	{
		char.stopIdle = IsReset;
		char.skipDance = IsReset;
		char.nonanimated = IsReset;
		char.specialAnim = IsReset;
		char.stunned = IsReset;
	}
	...
	public var isNoteHit:Bool = false;

	public function moveCameraXY(char:Character = null, isNoteHit:Bool = false, isDad:Bool = false, isGf:Bool = false, ?note:Note = null,
			?intensity1:Float = 60, ?intensity2:Float = 50):Void
	{
		intensity1 = ClientPrefs.cameraMovementX;
		intensity2 = ClientPrefs.cameraMovementY;

		if ((char.animOffsets.exists('singLEFT') && char.animOffsets.exists('singDOWN') && char.animOffsets.exists('singUP')
			&& char.animOffsets.exists('singRIGHT'))
			&& !isNoteHit)
		{
			if (isDad)
			{
		...
		else if ((!char.animOffsets.exists('singLEFT')
			&& !char.animOffsets.exists('singDOWN')
			&& !char.animOffsets.exists('singUP')
			&& !char.animOffsets.exists('singRIGHT'))
			&& isNoteHit)
		{
			if (isDad)
		...
		callOnLuas('onCameraMovement', [char, isNoteHit, isDad, isGf, note.noteData, intensity1, intensity2]);
	}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (!ClientPrefs.getGameplaySetting('practice', false)
						...
						&& !ClientPrefs.getGameplaySetting('notITGMod', true))
					{
				...
				else
				{
					...
					Debug.logInfo('LOADING NEXT SONG');
					Debug.logInfo(Paths.formatToSongPath(PlayState.storyPlaylist[0]) + difficulty);
	...
	function startAchievement(achieve:String)
	{
		...
		add(achievementObj);
		Debug.logInfo('Giving achievement ' + achieve);
	}
	...
	private function cachePopUpScore()
	{
		...
		Paths.image(pixelShitPart1 + "swag" + pixelShitPart2);
	...
	public var ratingAlpha:Float = 1;

	private function popUpScore(note:Note = null):Void
	{
		var noteDiff:Float = Math.abs(note.strumTime - Conductor.songPosition + ClientPrefs.ratingOffset);
		// Debug.logInfo(noteDiff, ' ' + Math.abs(note.strumTime - Conductor.songPosition));
		...
		if ((daRating.noteSplash && !note.noteSplashDisabled) && !SONG.notITG)
		{
		...
		switch (curStage)
		{
			default:
				offsetX = BF_X;
				offsetY = BF_Y;
				pixelShitPart1 = stagePixelShitPart1;
				pixelShitPart2 = stagePixelShitPart2;
		}

		var notPixelated = !(isPixelStage || pixelShitPart2.contains('pixel'));
		var isPixelated = (isPixelStage || pixelShitPart2.contains('pixel'));

		if (ClientPrefs.stillCombo)
		{
			var ratingPath:String = pixelShitPart1 + daRating.image + pixelShitPart2;
			...
			for (i in [rating])
			{
				i.setGraphicSize(Std.int(i.width * (notPixelated ? 0.7 : 1) * (isPixelated ? (daPixelZoom * 0.85) : 1)));
				i.antialiasing = notPixelated;

				i.updateHitbox();
				i.visible = (!ClientPrefs.hideHud && showRating);
				i.alpha = ratingAlpha;
			}

			comboGroup.add(rating);

			var comboPath:String = pixelShitPart1 + 'combo' + pixelShitPart2;

			var comboSpr:FlxSprite = comboGroup.recycle(FlxSprite).loadGraphic(Paths.image(comboPath));
			CoolUtil.resetSprite(comboSpr, 43 - 90 - offsetX + 680, 450 + (130 + offsetY));
			...
			comboSpr.setGraphicSize(Std.int(comboSpr.width * (notPixelated ? 0.7 : 1) * (isPixelated ? (daPixelZoom * 0.85) : 1)));
			comboSpr.antialiasing = notPixelated;
			...
			if (combo == 0 || combo >= 10)
			{
				...
				for (i in 0...separatedScore.length)
				{
					...
					var numScore:FlxSprite = comboGroup.recycle(FlxSprite).loadGraphic(Paths.image(ratingPath));
					...
					numScore.setGraphicSize(Std.int(numScore.width * (isPixelated ? daPixelZoom : 0.5)));
					numScore.antialiasing = notPixelated;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		// Debug.logInfo('pressed: ' + controlArray);
	}
			...
			if (parsedArray.contains(true))
			{
				for (i in 0...parsedArray.length)
				{
					if (parsedArray[i] && strumsBlocked[i] != true)
					{
						...
						callOnLuas('keyPressed', [parsedArray[i]]);
					}
				}
			}
		}*/
			...
			if (parsedArray.contains(true))
			{
				for (i in 0...parsedArray.length)
				{
					if (parsedArray[i] || strumsBlocked[i] == true)
					{
						...
						callOnLuas('keyRelease', [parsedArray[i]]);
					}
				}
			}
		}*/
	...
	{
		var BFconditions:Bool = (boyfriend.animation.curAnim != null
			&& boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / inst.pitch) * boyfriend.singDuration);
		var DADconditions:Bool = (dad.animation.curAnim != null
			&& dad.holdTimer > Conductor.stepCrochet * (0.0011 / inst.pitch) * dad.singDuration);
		...
		for (value in modchartCharacters.keys())
		{
			...
			var daCharConditions:Bool = (daChar.animation.curAnim != null
				&& daChar.holdTimer > Conductor.stepCrochet * (0.0011 / inst.pitch) * daChar.singDuration);
	...
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		// Debug.logInfo(daNote.missHealth);
		songMisses++;
		vocals.volume = 0;
		if (!practiceMode)
			songScore -= 10;
		...
		if (daNote != null)
		{
			...
			dType = SONG.notes[Std.int(curStep / 16)].dType;

		if (char != null && !daNote.noAnimation && !char.specialAnim && !daNote.noMissAnimation && char.hasMissAnimations)
		{
			var animToPlay:String = 'sing' + Note.keysShit.get(mania).get('anims')[daNote.noteData] + 'miss' + daNote.animSuffix;
			char.playAnim(animToPlay, true);
		}

		callOnLuas('playerOneMiss', [daNote.noteData, Conductor.songPosition, dType]);
	...
	function noteMissPress(direction:Int = 1):Void // You pressed a key when there was no notes to press for this key
	{
		...
		if (!char.stunned)
		{
			...
			if (combo > 5 && gf != null && gf.animOffsets.exists('sad') && !opponentMode)
			{
				gf.playAnim('sad');
			}
			combo = 0;
		...
		callOnLuas('playerOneMissPress', [direction, Conductor.songPosition, dType]);
	...
	public function opponentNoteHit(note:Note):Void
	{
		...
		if ((!note.noteSplashDisabled && !note.isSustainNote) && !SONG.notITG)
			spawnNoteSplashOnNoteCPU(note);

		var singData:Int = Std.int(Math.abs(note.noteData)) % Note.ammo[mania];
		...
		{
			moveCameraXY(char, true, isDadCam, isGfCam, note, 0, 0);
		}
		...
		StrumPlayAnim(true, singData, time);
		...
		callOnLuas('playerTwoSing', [Math.abs(note.noteData), Conductor.songPosition, note.dType]);
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			var songLightUp:Bool = (cpuControlled || chartingMode);
			...
			if (songLightUp)
			{
				...
				StrumPlayAnim(false, singData, time);
			}
			else
			{
	...
	var strumsAlpha:Float;
	var strumsAlphaCPU:Float;
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null)
	{
		...
		{
			playerStrums.forEachAlive(function(spr)
			{
	...
	public function spawnNoteSplashCPU(x:Float, y:Float, data:Int, ?note:Note = null)
	{
		...
		splashCPU.setupNoteSplash(x, y, data, skin, hue, sat, brt);

		for (i in opponentStrums)
		{
			opponentStrums.forEachAlive(function(spr)
			{
	...
	override function beatHit()
	{
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % camZoomingMult == 0)
		{
		...
		if (boyfriend != null
			&& idleToBeat
			&& curBeat % boyfriend.danceEveryNumBeats == 0
			&& boyfriend.animation.curAnim != null
			&& !boyfriend.animation.curAnim.name.startsWith('sing')
			&& !boyfriend.specialAnim
			&& !boyfriend.stunned)
		...
		for (value in modchartCharacters.keys())
		{
			...
			if (daChar != null
				&& idleToBeat
				&& curBeat % daChar.danceEveryNumBeats == 0
				&& daChar.animation.curAnim != null
				&& !daChar.animation.curAnim.name.startsWith('sing')
				&& !daChar.specialAnim
				&& !daChar.stunned)
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			setOnLuas('CPUAltAnim', SONG.notes[curSection].CPUAltAnim);
			setOnLuas('playerAltAnim', SONG.notes[curSection].playerAltAnim);
	...
	public function RecalculateRating(badHit:Bool = false)
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			else
			{
				...
				// Debug.logInfo((totalNotesHit / totalPlayed) + ', Total: ' + totalPlayed + ', notes hit: ' + totalNotesHit);
