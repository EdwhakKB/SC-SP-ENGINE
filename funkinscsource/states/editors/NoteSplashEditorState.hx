...
class NoteSplashEditorState extends MusicBeatState
{
  ...
  override function create()
  {
    ...
    for (i in 0...4)
    {
      var babyArrow:StrumArrow = new StrumArrow(-273, 50, i % 4, 1);
  ...
  function addAnimTab()
  {
    ...
    addButton = new PsychUIButton(20, 185, "Add/Update", function() {
      ...
      config = NoteSplash.addAnimationToConfig(config, scaleNumericStepper.value, name_input.text, prefix_input.text, [cast minFps.value, cast maxFps.value],
        offsets, indices, cast numericStepperData.value);
    ...
    reloadImage = function() {
      ...
      try
      {
        image = Paths.image("noteSplashes/" + imageSkin);
      }
      catch (e)
      {
        if (image == null)
      ...
      if (image == null)
      {
        errorText.text = "ERROR! Couldn't find 'noteSplashes/' or '' " + imageSkin + ".png";
        errorText.alpha = 1;
        return;
      }
      else
      {
        ...
        errorText.alpha = 1;
        errorText.text = "Succesfully loaded 'noteSplashes/' or '' " + imageSkin + ".png";
      }
      NoteSplash.configs.clear();
  ...
  override function update(elapsed:Float)
  {
    ...
    if (!blockInput)
    {
      if (controls.BACK) MusicBeatState.switchState(new MasterEditorMenu());
      if (FlxG.keys.justPressed.F1) openSubState(new NoteSplashEditorHelpSubState());
    }

    if (FlxG.mouse.overlaps(strums))
    {
      strums.forEach(function(strum:StrumArrow) {
        if (FlxG.mouse.overlaps(strum))
        {
          ...
          else
          {
            ...
            if (!splash.noteDataMap.exists(int) && splash.noteDataMap.exists(strum.ID % 4 + 4)) int = strum.ID % 4 + 4;
  ...
  {
    resetRGB();
    if (config.rgb != null) for (i in 0...config.rgb.length)
    {
      if (i > 2) break;
  ...
  public static function parseTxt(content:String):NoteSplashConfig
  {
    ...
    if (configs.length < 2 || configs[0].trim() == "") return config;
