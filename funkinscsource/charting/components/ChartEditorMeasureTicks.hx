...
class ChartEditorMeasureTicks extends FlxTypedSpriteGroup<FlxSprite>
{
  ...
  function updateMeasureNumber()
  {
    ...
    var measureNumberInViewport = Math.floor(viewTopPosition / ChartEditorState.GRID_SIZE / Conductor.instance.stepsPerSection) + 1;
    var measureNumberPosition = measureNumberInViewport * ChartEditorState.GRID_SIZE * Conductor.instance.stepsPerSection;
    ...
    if ((measureNumberInViewport + 1) < chartEditorState.songLengthInSteps / Conductor.instance.stepsPerSection)
      measureNumber.text = '${measureNumberInViewport + 1}';
    else
      measureNumber.text = '';

    // Debug.logInfo(measureNumber.text + ' at ' + measureNumber.y);
  }
