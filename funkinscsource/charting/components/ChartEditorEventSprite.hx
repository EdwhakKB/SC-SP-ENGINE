...
class ChartEditorEventSprite extends FunkinSCSprite
{
  ...
  static function buildFrames(force:Bool = false):FlxFramesCollection
  {
    ...
    for (eventName in backend.SafeNullArray.getEvents())
    {
      var exists:Bool = #if MODS_ALLOWED FileSystem.exists(Paths.getPath('ui/chart-editor/events/$eventName.png')) || #end Assets.exists(Paths.getPath('ui/chart-editor/events/$eventName.png'));
  ...
  {
    var eventNames:Array<String> = [DEFAULT_EVENT].concat(backend.SafeNullArray.getEvents());
  ...
  {
    if (name == null) name = eventData?.name ?? DEFAULT_EVENT;
  ...
  public function updateTooltipText():Void
  {
    ...
    this.tooltip.tipData = {text: this.eventData.name};
  }
