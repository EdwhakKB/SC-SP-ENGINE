...
class ChartEditorUploadVocalsDialog extends ChartEditorBaseDialog
{
  ...
  function buildDropHandlers():Void
  {
    ...
    {
      Debug.logInfo('Adding vocal upload for character ${charKey}');

      var charMetadata:Null<objects.Character.CharacterFile> = getFromCharacter(charKey);
      var charName:String = (charMetadata != null && charMetadata.vocals_file != null) ? charMetadata.vocals_file : "";
      ...
      var onDropFile:String->Void = function(pathStr:String) {
        Debug.logInfo('Selected file: $pathStr');
