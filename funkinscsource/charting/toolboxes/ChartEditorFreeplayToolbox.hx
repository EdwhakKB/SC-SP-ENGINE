...
import charting.commands.SetFreeplayPreviewCommand;
import ui.haxeui.components.WaveformPlayer;
...
class ChartEditorFreeplayToolbox extends ChartEditorBaseToolbox
{
  ...
  function initialize():Void
  {
    ...
    waveformScrollview.onScroll = (_) -> {
      if (!audioPreviewTracks.playing)
      {
        ...
        Debug.logInfo('newPos: ${playheadRelativePos}');
  ...
  public function refreshAudioPreview():Void
  {
    ...
    Debug.logInfo('Waveform data merging took: ${TimerUtil.seconds(perfStart)}');
  ...
  public function zoomWaveformIn():Void
  {
    ...
    if (waveformScale > MIN_SCALE)
    {
      ...
      Debug.logInfo('Zooming in, scale: ${waveformScale}');
  ...
  public function zoomWaveformOut():Void
  {
    ...
    Debug.logInfo('Zooming out, scale: ${waveformScale}');
  ...
  public function performPreview():Void
  {
    ...
    audioPreviewTracks.fadeIn(0.5, 0.25 * freeplayPreviewVolume, 1.0 * freeplayPreviewVolume, null);
  }
  ...
  public override function update(elapsed:Float)
  {
    ...
    if (isPerformingPreview && audioPreviewTracks.playing)
    {
      var startFadeOutTime = chartEditorState.currentSongFreeplayPreviewEnd - (0.25 * Constants.MS_PER_SEC);
      Debug.logInfo('startFadeOutTime: ${audioPreviewTracks.time} >= ${startFadeOutTime}');
      if (!isFadingOutPreview && audioPreviewTracks.time >= startFadeOutTime)
      {
        isFadingOutPreview = true;
        audioPreviewTracks.fadeOut(0.25, 1.0 * freeplayPreviewVolume, (_) -> {
          Debug.logInfo('Stop performing preview! ${audioPreviewTracks.time}');
          stopPerformingPreview();
        });
      }
    }
