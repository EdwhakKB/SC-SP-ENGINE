...
class ChartEditorAudioHandler
{
  ...
  {
    var trackData:Null<Bytes> = getBytes(path);
    if (trackData != null)
    {
  ...
  {
    return lime.utils.Bytes.fromFile(path);
  }
  ...
  public static function switchToInstrumental(state:ChartEditorState, instId:String = '', playerId:String, opponentId:String):Bool
  {
    ...
    Debug.logInfo('Switched to instrumental in ${TimerUtil.seconds(perfA, perfB)}.');
    Debug.logInfo('Stopped existing vocals in ${TimerUtil.seconds(perfB, perfC)}.');
    Debug.logInfo('Played BF vocals in ${TimerUtil.seconds(perfC, perfD)}.');
    Debug.logInfo('Played DAD vocals in ${TimerUtil.seconds(perfD, perfE)}.');
    Debug.logInfo('Hard refreshed offsets toolbox in ${TimerUtil.seconds(perfE, perfF)}.');
    Debug.logInfo('Hard refreshed freeplay toolbox in ${TimerUtil.seconds(perfF, perfG)}.');
   ...
   */
  public static function playVocals(state:ChartEditorState, charType:objects.Character.CharacterType, charId:String, instId:String = ''):Bool
  {
    var trackId:String = '${charId}${instId == '' ? '' : '-${instId}'}';
    ...
    if (vocalTrack != null)
    {
      switch (charType)
      {
        case BF:
          ...
          Debug.logInfo('Interpreted waveform data in ${TimerUtil.seconds(perfStart)}.');
