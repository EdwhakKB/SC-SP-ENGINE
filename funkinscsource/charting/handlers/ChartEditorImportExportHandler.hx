...
import utils.FileUtil.FileWriteMode;
...
import backend.song.data.importer.ChartManifestData;
...
class ChartEditorImportExportHandler
{
  ...
  public static function loadSongAsTemplate(state:ChartEditorState, songId:String, ?startingVariation:String, ?startingDifficulty:String):Void
  {
    ...
    for (metadata in rawSongMetadata)
    {
      ...
      var variation = (metadata.songData.playData.variation == null
        || metadata.songData.playData.variation == '') ? Constants.DEFAULT_VARIATION : metadata.songData.playData.variation;

      // Clone to prevent modifying the original.
      var metadataClone:SongMetaData = metadata.clone();
      metadataClone.songData.playData.variation = variation;
      if (metadataClone != null) songMetadata.set(variation, metadataClone);

      var chartData:Null<SongChartData> = SongRegistry.instance.parseEntryChartData(songId, metadata.songData.playData.variation);
    ...
    for (variation in variations)
    {
      for (difficultyId in song.listDifficulties(variation, true, true))
      {
        ...
        {
          state.loadInstFromAsset(Paths.getInstPath(songId, [diff.options.instrumentalPrefix, diff.options.instrumentalSuffix, true]));
        }
        else
        {
          state.loadInstFromAsset(Paths.getInstPath(songId, [diff.options.instrumentalPrefix, '-$variation', true]), variation);
        }

        var instId:String = diff.variation == Constants.DEFAULT_VARIATION ? '' : diff.variation;
        var voiceList:Array<String> = diff.buildVoiceListByString(); // SongDifficulty accounts for variation already.
  ...
  public static function loadFromFNFCPath(state:ChartEditorState, path:String):Null<Array<String>>
  {
    ...
    Debug.logInfo('Loaded ${bytes.length} bytes from $path');
  ...
  public static function loadFromFNFC(state:ChartEditorState, bytes:Bytes, ?startingVariation:String, ?startingDifficulty:String):Null<Array<String>>
  {
    ...
    var variationList:Array<String> = baseMetadata.songData.playData.songVariations;
    ...
    for (variation in [Constants.DEFAULT_VARIATION].concat(variationList))
    {
      ...
      var instId:String = variMetadata?.songData?.playData?.characters?.instrumental ?? '';
      var playerCharId:String = variMetadata?.songData?.playData?.characters?.player ?? Constants.DEFAULT_CHARACTER;
      var opponentCharId:Null<String> = variMetadata?.songData?.playData?.characters?.opponent;
      ...
      ?onCancelCb:Void->Void):Void
  {
    ...
    for (variation in variations)
    {
      ...
      if (variationId == '')
      {
        ...
        {
          variationMetadata.songData.playData.version = backend.song.data.SongRegistry.SONG_METADATA_VERSION;
          variationMetadata.songData.inclusiveData.generatedBy = backend.song.data.SongRegistry.DEFAULT_GENERATEDBY;
        ...
        {
          variationChart.version = backend.song.data.SongRegistry.SONG_CHART_DATA_VERSION;
          variationChart.generatedBy = backend.song.data.SongRegistry.DEFAULT_GENERATEDBY;
    ...
    Debug.logInfo('Exporting ${zipEntries.length} files to ZIP...');
    ...
    if (force)
    {
      ...
      if (targetPath == null)
      {
        ...
        targetMode = Skip;
        targetPath = Path.join([BACKUPS_PATH, 'chart-editor-${DateUtil.generateTimestamp()}.fnfc']);
        // We have to force write because the program will die before the save dialog is closed.
        Debug.logInfo('Force exporting to $targetPath...');
        try
        {
    ...
    else
    {
      ...
      var onSave:Array<String>->Void = function(paths:Array<String>) {
        ...
        {
          Debug.logInfo('Saved to "${paths[0]}"');
      ...
      Debug.logInfo('Exporting to user-defined location...');
      try
      {
        FileUtil.saveChartAsFNFC(zipEntries, onSave, onCancel, '${state.currentSongId}.fnfc');
        state.saveDataDirty = false;
      }
      catch (e) {}
    }
