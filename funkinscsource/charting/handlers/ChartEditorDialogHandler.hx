package charting.handlers;

import flixel.util.FlxTimer;
import backend.song.data.importer.FNFLegacyData;
import backend.song.data.importer.FNFLegacyImporter;
...
import charting.dialogs.ChartEditorAboutDialog;
import charting.dialogs.ChartEditorBaseDialog.DialogDropTarget;
import charting.dialogs.ChartEditorCharacterIconSelectorMenu;
import charting.dialogs.ChartEditorUploadChartDialog;
import charting.dialogs.ChartEditorWelcomeDialog;
import charting.dialogs.ChartEditorUploadVocalsDialog;
...
class ChartEditorDialogHandler
{
  ...
  {
    var charData:SongCharacterData = state.currentSongMetadata.songData.playData.characters;
   ...
   */
  public static function openCharacterDropdown(state:ChartEditorState, charType:objects.Character.CharacterType, lockPosition:Bool = false):Null<Menu>
  {
  ...
  public static function openUploadInstDialog(state:ChartEditorState, closable:Bool = true):Dialog
  {
    ...
    var onDropFile:String->Void = function(pathStr:String) {
      ...
      Debug.logInfo('Dropped file (${path})');
  ...
  public static function openSongMetadataDialog(state:ChartEditorState, erect:Bool, targetVariation:String, clearExistingMetadata:Bool):Dialog
  {
    ...
    newSongMetadata.songData.playData.variation = targetVariation;
    newSongMetadata.songData.playData.difficulties = (erect) ? ['erect', 'nightmare'] : ['easy', 'normal', 'hard'];
    ...
    inputSongName.onChange = function(event:UIEvent) {
      ...
      if (valid)
      {
        ...
        newSongMetadata.songData.playData.songName = event.target.text;
      }
      else
      {
        newSongMetadata.songData.playData.songName = "";
      }
    };
    inputSongName.text = "";
    ...
    inputSongArtist.onChange = function(event:UIEvent) {
      ...
      if (valid)
      {
        ...
        newSongMetadata.songData.inclusiveData.artist = event.target.text;
      }
      else
      {
        newSongMetadata.songData.inclusiveData.artist = "";
      }
    };
    ...
    inputStage.onChange = function(event:UIEvent) {
      ...
      newSongMetadata.songData.playData.stage = event.data.id;
    };
    var startingValueStage = ChartEditorDropdowns.populateDropdownWithStages(inputStage, newSongMetadata.songData.playData.stage);
    ...
    inputNoteStyle.onChange = function(event:UIEvent) {
      ...
      newSongMetadata.songData.playData.options.arrowSkin = event.data.id;
    };
    var startingValueNoteStyle = ChartEditorDropdowns.populateDropdownWithNoteStyles(inputNoteStyle, newSongMetadata.songData.playData.options.arrowSkin);
    ...
    inputCharacterPlayer.onChange = function(event:UIEvent) {
      if (event.data?.id == null) return;
      newSongMetadata.songData.playData.characters.player = event.data.id;
    };
    var startingValuePlayer = ChartEditorDropdowns.populateDropdownWithCharacters(inputCharacterPlayer, CharacterType.BF,
      newSongMetadata.songData.playData.characters.player);
    ...
    inputCharacterOpponent.onChange = function(event:UIEvent) {
      if (event.data?.id == null) return;
      newSongMetadata.songData.playData.characters.opponent = event.data.id;
    };
    var startingValueOpponent = ChartEditorDropdowns.populateDropdownWithCharacters(inputCharacterOpponent, CharacterType.DAD,
      newSongMetadata.songData.playData.characters.opponent);
    ...
    inputCharacterGirlfriend.onChange = function(event:UIEvent) {
      if (event.data?.id == null) return;
      newSongMetadata.songData.playData.characters.girlfriend = event.data.id == "none" ? "" : event.data.id;
    };
    var startingValueGirlfriend = ChartEditorDropdowns.populateDropdownWithCharacters(inputCharacterGirlfriend, CharacterType.GF,
      newSongMetadata.songData.playData.characters.girlfriend);
    ...
    dialogBPM.onChange = function(event:UIEvent) {
      ...
      var timeChanges:Array<SongTimeChange> = newSongMetadata.songData.playData.timeChanges;
      ...
      newSongMetadata.songData.playData.timeChanges = timeChanges;
    };
  ...
  public static function openChartDialog(state:ChartEditorState, closable:Bool = true):Dialog
  {
    ...
    onDropFileMetadataVariation = function(variation:String, label:Label, pathStr:String) {
      var path:Path = new Path(pathStr);
      Debug.logInfo('Dropped JSON file (${path})');
      ...
      if (variation == Constants.DEFAULT_VARIATION) constructVariationEntries(songMetadataVariation.songData.playData.songVariations);
    };
  ...
  public static function openImportChartDialog(state:ChartEditorState, format:String, closable:Bool = true):Null<Dialog>
  {
    ...
    importBox.onClick = function(_) {
      Dialogs.openBinaryFile('Import Chart - ${prettyFormat}', fileFilter != null ? [fileFilter] : [], function(selectedFile:SelectedFileInfo) {
        if (selectedFile != null && selectedFile.bytes != null)
        {
          Debug.logInfo('Selected file: ' + selectedFile.fullPath);
  ...
  public static function openAddVariationDialog(state:ChartEditorState, closable:Bool = true):Dialog
  {
    ...
    dialogSongName.value = state.currentSongMetadata.songData.playData.songName;
    ...
    dialogSongArtist.value = state.currentSongMetadata.songData.inclusiveData.artist;
    ...
    var startingValueStage = ChartEditorDropdowns.populateDropdownWithStages(dialogStage, state.currentSongMetadata.songData.playData.stage);
    ...
    dialogNoteStyle.value = state.currentSongMetadata.songData.playData.options.arrowSkin;
    ...
    dialogCharacterPlayer.value = ChartEditorDropdowns.populateDropdownWithCharacters(dialogCharacterPlayer, CharacterType.BF,
      state.currentSongMetadata.songData.playData.characters.player);
    ...
    dialogCharacterOpponent.value = ChartEditorDropdowns.populateDropdownWithCharacters(dialogCharacterOpponent, CharacterType.DAD,
      state.currentSongMetadata.songData.playData.characters.opponent);
    ...
    dialogCharacterGirlfriend.value = ChartEditorDropdowns.populateDropdownWithCharacters(dialogCharacterGirlfriend, CharacterType.GF,
      state.currentSongMetadata.songData.playData.characters.girlfriend);

    var dialogBPM:Null<NumberStepper> = dialog.findComponent('dialogBPM', NumberStepper);
    if (dialogBPM == null) throw 'Could not locate dialogBPM NumberStepper in Add Variation dialog';
    var currentStartingBPM:Float = state.currentSongMetadata.songData.playData.timeChanges[0].bpm;
    ...
    variationForm.onSubmit = function(_) {
      state.isHaxeUIDialogOpen = false;
      Debug.logInfo('Add Variation dialog submitted, validation succeeded!');
      ...
      pendingVariation.songData.playData.stage = dialogStage.value.id;
      pendingVariation.songData.playData.options.arrowSkin = dialogNoteStyle.value;
      pendingVariation.songData.playData.timeChanges[0].bpm = dialogBPM.value;

      state.songMetadata.set(pendingVariation.songData.playData.variation, pendingVariation);
      ...
      state.success('Add Variation', 'Added new variation "${pendingVariation.songData.playData.variation}"');
  ...
  public static function openAddDifficultyDialog(state:ChartEditorState, closable:Bool = true):Dialog
  {
    ...
    difficultyForm.onSubmit = function(_) {
      Debug.logInfo('Add Difficulty dialog submitted, validation succeeded!');
