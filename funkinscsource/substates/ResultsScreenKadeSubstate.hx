...
class ResultsScreenKadeSubstate extends substates.MusicBeatSubState
{
  ...
  public function new(follow:flixel.FlxObject)
  {
    ...
    songText = new FlxText(20, -65, FlxG.width,
      PlayState.isStoryMode ? '' : 'Played on ${PlayState.currentChart.songName} - [${Difficulty.getString().toLowerCase()}]');
    songText.size = 34;
...
=======
    ...
    if (superMegaConditionShit)
    {
      var percent:Float = game.ratingPercent;
      if (Math.isNaN(percent)) percent = 0;
      Highscore.saveScore(PlayState.currentChart.songName, game.songScore, PlayState.storyDifficulty, percent);
      Highscore.saveCombo(PlayState.currentChart.songName, game.ratingFC, PlayState.storyDifficulty);
      Highscore.saveLetter(PlayState.currentChart.songName, game.comboLetterRank, PlayState.storyDifficulty);
    }
  ...
  {
    var noteRating = note.rating;
    var noteDiff = note.strumTime - Conductor.instance.songPosition;
    var noteStrumTime = note.strumTime;
