...
class FunkinLua {
	...
	public function new(script:String, ?preloading:Bool = false, ?isStageLua:Bool = false) {
		#if LUA_ALLOWED
		lua = LuaL.newstate();
		LuaL.openlibs(lua);
		...
		Debug.logInfo('lua file loaded succesfully:' + script + ' Is Stage Lua ?: ' + isStageLua);
		...
		if (preloading) //only the necessary functions for preloading are included
		{
			...
			Lua_helper.add_callback(lua, "makeLuaBackdrop", function(tag:String, image:String, x:Float, y:Float, ?axes:String = "XY") {
				...
				if(image != null && image.length > 0) {
					leSprite = new FlxBackdrop(image, FlxAxes.fromString(axes), Std.int(x), Std.int(y));
				}
		...
		else
		{
			...
			Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false, place:Dynamic = false) {
				if (isStageLua)
				{
					if (Stage.instance.swagBacks.exists(tag))
					{
						...
						//if (!shit.wasAdded)
						//{
							if ((place == -1 || place == false || place == "false") || !front)
								Stage.instance.toAdd.push(shit);
							else
							{
								if ((place == true || place == "true") || front){place = 2;}
			...
			Lua_helper.add_callback(lua,"destroyObject", function(id:String, ?bg:Bool = false) {
				if (bg)
					PlayState.instance.Stage.destroyObj(PlayState.instance.Stage.swagBacks[id]);
				else
				{
					var shit:Dynamic = getObjectDirectly2(id);
					PlayState.instance.destroyObj(shit);
				}
			});
