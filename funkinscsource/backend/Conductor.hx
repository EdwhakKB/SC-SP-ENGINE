...
class Conductor
{
   ...
   */
  public static var sectionHit(default, null):FlxSignal = new FlxSignal();
   ...
   */
  public var onSectionHit(default, null):FlxSignal = new FlxSignal();
   ...
   */
  public var sectionLengthMs(get, never):Float;

  function get_sectionLengthMs():Float
  {
   ...
   */
  public var currentSection(default, null):Int = 0;
   ...
   */
  public var currentSectionTime(default, null):Float = 0;
  ...
  /**
   * The safe zone offset.
   */
  public var safeZoneOffset(get, never):Float;

  function get_safeZoneOffset():Float
  {
    return (ClientPrefs.data.safeFrames / 60) * 1000 * playbackRate;
  }

  /**
   * Playbackrate of the state.
   */
  ...
  {
    return ClientPrefs.data != null ? ClientPrefs.data.audioVisualOffset : 0;
  }

  function set_audioVisualOffset(value:Int):Int
  {
    ClientPrefs.data.audioVisualOffset = value;
    ClientPrefs.flush();
    return ClientPrefs.data.audioVisualOffset;
  }

  /**
   * The number of beats in a measure. May be fractional depending on the time signature.
   */
  public var beatsPerSection(get, never):Float;

  function get_beatsPerSection():Float
  {
    // NOTE: Not always an integer, for example 7/8 is 3.5 beats per measure
    return stepsPerSection / Constants.STEPS_PER_BEAT;
  }
   ...
   */
  public var stepsPerSection(get, never):Int;

  function get_stepsPerSection():Int
  {
  ...
  static function dispatchSectionHit():Void
  {
  ...
  {
    input.onSectionHit.add(dispatchSectionHit);
  ...
  {
    input.onSectionHit.remove(dispatchSectionHit);
  ...
  public function forceBPM(?bpm:Float):Void
  {
    if (bpm != null)
    {
      Debug.logInfo('[CONDUCTOR] Forcing BPM to ${bpm}');
    }
    else
    {
  ...
  public function update(?songPos:Float, applyOffsets:Bool = true, forceDispatch:Bool = false)
  {
    ...
    var oldSection:Float = this.currentSection;
    ...
    else if (currentTimeChange != null && this.songPosition > 0.0)
    {
      ...
      this.currentSectionTime = currentStepTime / stepsPerSection;
      this.currentStep = Math.floor(currentStepTime);
      this.currentBeat = Math.floor(currentBeatTime);
      this.currentSection = Math.floor(currentSectionTime);
    }
    else
    {
  ...
  public function mapTimeChanges(songTimeChanges:Array<SongTimeChange>):Void
  {
    ...
    {
      Debug.logInfo('Done mapping time changes: ${timeChanges}');
    }
   ...
   * @return The time in steps.
   */
  public function getTimeInBeats(ms:Float):Float
  {
    if (timeChanges.length == 0)
    {
      // Assume a constant BPM equal to the forced value.
      return Math.floor(ms / beatLengthMs);
    }
    else
    {
      var resultStep:Float = 0;
  ...
  public static function watchQuick(?target:Conductor):Void
  {
    ...
    FlxG.watch.addQuick('currentSectionTime', target.currentSectionTime);
