...
class Tank extends BaseStage
{
  ...
  override public function buildStage(baseStage:Stage)
  {
    ...
    baseStage.stageSpriteHandler(sky, -1, 'tankSky');
    ...
    if (!ClientPrefs.data.lowQuality)
    {
      ...
      baseStage.stageSpriteHandler(clouds, -1, 'tankClouds');

      var mountains:BGSprite = new BGSprite('tankMountains', -300, -20, 0.2, 0.2);
      mountains.setGraphicSize(Std.int(1.2 * mountains.width));
      mountains.updateHitbox();
      baseStage.stageSpriteHandler(mountains, -1, 'tankMountains');

      var buildings:BGSprite = new BGSprite('tankBuildings', -200, 0, 0.3, 0.3);
      buildings.setGraphicSize(Std.int(1.1 * buildings.width));
      buildings.updateHitbox();
      baseStage.stageSpriteHandler(buildings, -1, 'tankBuildings');
    }

    var ruins:BGSprite = new BGSprite('tankRuins', -200, 0, .35, .35);
    ruins.setGraphicSize(Std.int(1.1 * ruins.width));
    ruins.updateHitbox();
    baseStage.stageSpriteHandler(ruins, -1, 'tankRuins');

    if (!ClientPrefs.data.lowQuality)
    {
      var smokeLeft:BGSprite = new BGSprite('smokeLeft', -200, -100, 0.4, 0.4, ['SmokeBlurLeft'], true);
      baseStage.stageSpriteHandler(smokeLeft, -1, 'smokeLeft');
      var smokeRight:BGSprite = new BGSprite('smokeRight', 1100, -100, 0.4, 0.4, ['SmokeRight'], true);
      baseStage.stageSpriteHandler(smokeRight, -1, 'smokeRight');

      tankWatchtower = new BGSprite('tankWatchtower', 100, 50, 0.5, 0.5, ['watchtower gradient color']);
      baseStage.stageSpriteHandler(tankWatchtower, -1, 'tankWatchtower');
    }

    tankGround = new BackgroundTank();
    baseStage.stageSpriteHandler(tankGround, -1, 'backgroundTank');

    tankmanRun = new FlxTypedGroup<TankmenBG>();
    baseStage.stageSpriteHandler(tankmanRun, -1, 'tankmanRun');
    baseStage.setSwagGroup('tankmanRun', tankmanRun);

    var ground:BGSprite = new BGSprite('tankGround', -420, -150);
    ground.setGraphicSize(Std.int(1.15 * ground.width));
    ground.updateHitbox();
    baseStage.stageSpriteHandler(ground, -1, 'tankGround');
    ...
    baseStage.stageSpriteHandler(foregroundSprites, 4, 'foregroundSprites');
    baseStage.setSwagGroup('foregroudSprites', foregroundSprites);
