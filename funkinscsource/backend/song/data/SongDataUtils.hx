...
class SongDataUtils
{
  ...
  public static function offsetSongEventData(events:Array<SongEventData>, offset:Float):Array<SongEventData>
  {
    return events.map(function(event:SongEventData):SongEventData {
      return new SongEventData(event.time + offset, event.name, event.value);
    });
  }
  ...
  public static function flipNotes(notes:Array<SongNoteData>, ?strumlineSize:Int = 4):Array<SongNoteData>
  {
    return notes.map(function(note:SongNoteData):SongNoteData {
      ...
      return new SongNoteData(note.time, newData, note.length, note.type);
    });
  }
  ...
  public static function writeItemsToClipboard(data:SongClipboardItems):Void
  {
    ...
    Debug.logInfo('Wrote ' + data.notes.length + ' notes and ' + data.events.length + ' events to clipboard.');
  }
  ...
  public static function readItemsFromClipboard():SongClipboardItems
  {
    ...
    Debug.logInfo('Read ${notesString.length} characters from clipboard.');
    ...
    else
    {
      ...
      Debug.logInfo('Parsed ' + data.notes.length + ' notes and ' + data.events.length + ' from clipboard.');
      data.valid = true;
      return data;
    }
  }
   ...
   */
  public static function getEventsWithNames(events:Array<SongEventData>, names:Array<String>):Array<SongEventData>
  {
    return events.filter(function(event:SongEventData):Bool {
