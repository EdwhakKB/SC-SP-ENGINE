...
import charting.ChartEditorState.ChartEditorTheme;
import charting.ChartEditorState.ChartEditorLiveInputStyle;
...
@:structInit class SaveVariables
{
  ...
  /**
   * Contains all the user's options and preferences, specific to the Chart Editor.
   */
  public var chartEditorSettings:Map<String, Dynamic> = [
    "chartEditorPreviousFiles" => [],
    "chartEditorHasBackup" => false,
    "chartEditorDownscroll" => false,
    "chartEditorPlaytestStartTime" => false,
    "chartEditorTheme" => ChartEditorTheme.Dark,
    "chartEditorThemeMusic" => false,
    "chartEditorMetronomeVolume" => 0.0,
    "chartEditorHitsoundVolumePlayer" => 0.0,
    "chartEditorHitsoundVolumeOpponent" => 0.0,
    "chartEditorPlaybackSpeed" => 1.0,
...
class ClientPrefs
{
  ...
  public static function loadPrefs()
  {
    ...
    for (key in Reflect.fields(data))
      if (key != 'gameplaySettings' && key != "chartEditorSettings" && Reflect.hasField(FlxG.save.data,
        key)) Reflect.setField(data, key, Reflect.field(FlxG.save.data, key));
    ...
    if (FlxG.save.data.chartEditorSettings != null)
    {
      var savedMap:Map<String, Dynamic> = FlxG.save.data.chartEditorSettings;
      for (name => value in savedMap)
        data.chartEditorSettings.set(name, value);
    }
  ...
  inline public static function getChartEditorSetting(name:String, defaultValue:Dynamic = null, ?customDefaultValue:Bool = false):Dynamic
  {
    if (!customDefaultValue) defaultValue = defaultData.chartEditorSettings.get(name);
    return /*PlayState.isStoryMode ? defaultValue : */ (data.chartEditorSettings.exists(name) ? data.chartEditorSettings.get(name) : defaultValue);
  }
