...
import scfunkin.states.substates.options.ModSettingsSubState;
...
class ModsMenuState extends MusicBeatState
{
  ...
  override function create()
  {
    ...
    bgTitle = FlxSpriteUtil.drawRoundRectComplex(new FlxSprite(bgList.x + bgList.width + 20, 40).makeGraphic(840, 180, FlxColor.TRANSPARENT), 0, 0, 840, 180,
      15, 15, 0, 0, FlxColor.BLACK);
    bgTitle.alpha = 0.6;
    add(bgTitle);
    ...
    bgDescription = FlxSpriteUtil.drawRoundRectComplex(new FlxSprite(bgTitle.x, bgTitle.y + 200).makeGraphic(840, 450, FlxColor.TRANSPARENT), 0, 0, 840, 450,
      0, 0, 15, 15, FlxColor.BLACK);
    ...
    bgButtons = FlxSpriteUtil.drawRoundRectComplex(new FlxSprite(bgDescription.x,
      bgDescription.y + bgDescription.height - myHeight).makeGraphic(840, myHeight, FlxColor.TRANSPARENT), 0, 0, 840, myHeight,
      0, 0, 15, 15, FlxColor.WHITE);
            ...
            if (curMod != null)
            {
              ...
              if (holdingMod)
              {
                ...
                if (!moved)
                {
                  ...
                  else if (FlxG.mouse.y > bgList.y + bgList.height) factor = Math.abs(Math.max(0.2,
                    Math.min(0.5, 0.5 - (FlxG.mouse.y - bgList.y - bgList.height) / 100)));
  ...
  function updateItemPositions()
  {
    ...
    for (i => mod in modsGroup.members)
    {
      if (mod == null)
      {
        Debug.logError('Mod #$i is null, maybe it was ' + modsList.all[i]);
        continue;
      }
