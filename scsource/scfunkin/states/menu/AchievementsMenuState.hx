...
class AchievementsMenuState extends MusicBeatState
{
  ...
  override function create()
  {
    ...
    {
      var hasAntialias:Bool = Save.get('antialiasing');
      var graphic = null;
      if (option.unlocked)
      {
  ...
  function makeAchievement(achievement:String, data:Achievement, unlocked:Bool, mod:String = null)
  {
    return {
      name: achievement,
      displayName: unlocked ? Language.getPhrase('description_$achievement', data.description) : '???',
  ...
  function _changeSelection()
  {
    ...
    if (hasProgress)
    {
      ...
      progressTxt.text = scfunkin.utils.CoolUtil.floorDecimal(val1, options[curSelected].decProgress)
        + ' / '
        + scfunkin.utils.CoolUtil.floorDecimal(val2, options[curSelected].decProgress);
  ...
  override function update(elapsed:Float)
  {
    ...
    if (controls.ACCEPT)
    {
      if (onYes)
      {
        ...
        state.grpOptions.members[state.curSelected].antialiasing = Save.get('antialiasing');
