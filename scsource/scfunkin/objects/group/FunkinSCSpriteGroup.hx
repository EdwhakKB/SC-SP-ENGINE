...
/**
 * Made by glowsoony
 * `FunkinSCSpriteGroup` is a special `FunkinSCSprite` that can be treated like a single sprite even if it's
 * made up of several member sprites. It shares the `FlxGroup` API, but it doesn't inherit from it.
 * Note that `FunkinSCSpriteContainer` also exists.
 *
 ...
 */
typedef FunkinSCSpriteGroup = FunkinSCTypedSpriteGroup<FunkinSCSprite>;

/**
 * A `FunkinSCSpriteGroup` that only allows specific members to be a specific type of `FunkinSCSprite`.
 * To use any kind of `FunkinSCSprite` use `FunkinSCSpriteGroup`, which is an alias for
 * `FunkinSCTypedSpriteGroup<FunkinSCSprite>`.
 */
class FunkinSCTypedSpriteGroup<T:FunkinSCSprite> extends FunkinSCSprite
{
  /**
  ...
  /**
   * Array of all the `FunkinSCSprite`s that exist in this group for
   * optimization purposes / static typing on cpp targets.
   */
  @:deprecated("_sprites is deprecated, use group.members")
  var _sprites(get, never):Array<FunkinSCSprite>;
   ...
   */
  override public function clone():FunkinSCTypedSpriteGroup<T>
  {
    var newGroup = new FunkinSCTypedSpriteGroup<T>(x, y, maxSize);
  ...
  /**
   * Checks to see if a point in 2D world space overlaps any `FunkinSCSprite` object from this group.
   *
  ...
  /**
   * Checks to see if a point in 2D world space overlaps any of FunkinSCSprite object's current displayed pixels.
  ...
  /**
   * Adds a new `FunkinSCSprite` subclass to the group.
   *
  ...
  /**
   * Inserts a new `FunkinSCSprite` subclass to the group at the specified position.
   *
   ...
   *
   * @param   ObjectClass     The class type you want to recycle (e.g. `FunkinSCSprite`, `EvilRobot`, etc).
   ...
   *
   * @param   sprite  The `FunkinSCSprite` you want to remove.
  ...
  /**
   * Replaces an existing `FunkinSCSprite` with a new one.
   *
   ...
   * @param   Force         Force the object to be an `ObjectClass` and not a super class of `ObjectClass`.
   * @return  A `FunkinSCSprite` currently flagged as not existing.
   */
   ...
   *
   * @return  A `FunkinSCSprite` currently flagged as existing.
   */
   ...
   * This is handy for checking if everything's wiped out, or choosing a squad leader, etc.
   *
   * @return  A `FunkinSCSprite` currently flagged as not dead.
   */
   ...
   * This is handy for checking if everything's wiped out, or choosing a squad leader, etc.
   *
   * @return  A `FunkinSCSprite` currently flagged as dead.
   */
   ...
   *
   * @return  The number of `FunkinSCSprite`s flagged as not dead. Returns `-1` if group is empty.
   */
   ...
   *
   * @return  The number of `FunkinSCSprite`s flagged as dead. Returns `-1` if group is empty.
   */
   ...
   * @return  A `FunkinSCSprite` from the `members` list.
   */
   ...
   *
   * @param   ObjectClass   A class that objects will be checked against before Function is applied, ex: `FunkinSCSprite`.
   * @param   Function      A function that modifies one element at a time.
   * @param   Recurse       Whether or not to apply the function to members of subgroups as well.
   */
  ...
  /**
   * Remove all instances of `FunkinSCSprite` from the list.
  ...
  function findMinXHelper()
  {
    ...
    for (member in group.members)
    {
      ...
      var minX:Float;
      if (member.flixelType == SPRITEGROUP) minX = (cast member : FunkinSCSpriteGroup).findMinX();
      else
        minX = member.x;
  ...
  function findMaxXHelper()
  {
    ...
    for (member in group.members)
    {
      ...
      var maxX:Float;
      if (member.flixelType == SPRITEGROUP) maxX = (cast member : FunkinSCSpriteGroup).findMaxX();
      else
  ...
  function findMinYHelper()
  {
    ...
    for (member in group.members)
    {
      ...
      var minY:Float;
      if (member.flixelType == SPRITEGROUP) minY = (cast member : FunkinSCSpriteGroup).findMinY();
      else
        minY = member.y;
  ...
  function findMaxYHelper()
  {
    ...
    for (member in group.members)
    {
      ...
      var maxY:Float;
      if (member.flixelType == SPRITEGROUP) maxY = (cast member : FunkinSCSpriteGroup).findMaxY();
      else
  ...
  inline function xTransform(Sprite:FunkinSCSprite, X:Float)
    Sprite.x += X; // addition

  inline function yTransform(Sprite:FunkinSCSprite, Y:Float)
    Sprite.y += Y; // addition

  inline function angleTransform(Sprite:FunkinSCSprite, Angle:Float)
    Sprite.angle += Angle; // addition

  inline function alphaTransform(Sprite:FunkinSCSprite, Alpha:Float)
  {
  ...
  inline function directAlphaTransform(Sprite:FunkinSCSprite, Alpha:Float)
    Sprite.alpha = Alpha; // direct set

  inline function facingTransform(Sprite:FunkinSCSprite, Facing:FlxDirectionFlags)
    Sprite.facing = Facing;

  inline function flipXTransform(Sprite:FunkinSCSprite, FlipX:Bool)
    Sprite.flipX = FlipX;

  inline function flipYTransform(Sprite:FunkinSCSprite, FlipY:Bool)
    Sprite.flipY = FlipY;

  inline function movesTransform(Sprite:FunkinSCSprite, Moves:Bool)
    Sprite.moves = Moves;

  inline function pixelPerfectTransform(Sprite:FunkinSCSprite, PixelPerfect:Bool)
    Sprite.pixelPerfectRender = PixelPerfect;

  inline function gColorTransform(Sprite:FunkinSCSprite, Color:Int)
    Sprite.color = Color;

  inline function blendTransform(Sprite:FunkinSCSprite, Blend:BlendMode)
    Sprite.blend = Blend;

  inline function immovableTransform(Sprite:FunkinSCSprite, Immovable:Bool)
    Sprite.immovable = Immovable;

  inline function visibleTransform(Sprite:FunkinSCSprite, Visible:Bool)
    Sprite.visible = Visible;

  inline function activeTransform(Sprite:FunkinSCSprite, Active:Bool)
    Sprite.active = Active;

  inline function solidTransform(Sprite:FunkinSCSprite, Solid:Bool)
    Sprite.solid = Solid;

  inline function aliveTransform(Sprite:FunkinSCSprite, Alive:Bool)
    Sprite.alive = Alive;

  inline function existsTransform(Sprite:FunkinSCSprite, Exists:Bool)
    Sprite.exists = Exists;

  inline function cameraTransform(Sprite:FunkinSCSprite, Camera:FlxCamera)
    Sprite.camera = Camera;

  inline function camerasTransform(Sprite:FunkinSCSprite, Cameras:Array<FlxCamera>)
    Sprite.cameras = Cameras;

  inline function offsetTransform(Sprite:FunkinSCSprite, Offset:FlxPoint)
    Sprite.offset.copyFrom(Offset);

  inline function originTransform(Sprite:FunkinSCSprite, Origin:FlxPoint)
    Sprite.origin.copyFrom(Origin);

  inline function scaleTransform(Sprite:FunkinSCSprite, Scale:FlxPoint)
    Sprite.scale.copyFrom(Scale);

  inline function skewTransform(Sprite:FunkinSCSprite, Skew:FlxPoint)
    Sprite.skew.copyFrom(Skew);

  inline function scrollFactorTransform(Sprite:FunkinSCSprite, ScrollFactor:FlxPoint)
    Sprite.scrollFactor.copyFrom(ScrollFactor);

  inline function clipRectTransform(Sprite:FunkinSCSprite, ClipRect:FlxRect)
  {
  ...
  {
    #if FLX_DEBUG
    throw "This function is not supported in FunkinSCSpriteGroup";
    #end
    return this;
  }
   ...
   * @return this sprite group
   */
  override public function makeGraphic(Width:Int, Height:Int, Color:Int = FlxColor.WHITE, Unique:Bool = false, ?Key:String):FunkinSCSprite
  {
    #if FLX_DEBUG
    throw "This function is not supported in FunkinSCSpriteGroup";
    #end
  ...
  inline function get__sprites():Array<FunkinSCSprite>
  {
   ...
   * This functionality isn't supported in SpriteGroup
   */
  override function drawStage(camera:FlxCamera) {}

  /**
   * This functionality isn't supported in SpriteGroup
   */
  override function loadFrameAtlas(path:String, ?parentfolder:String) {}
}
