 ...
 * Since `FlxSkewedSpriteGroups` and `FlxSkewedSpriteContainers` are usually meant to draw groups of sprites
 ...
 */
typedef FlxSkewedSpriteGroup = FlxSkewedTypedSpriteGroup<FlxSkewed>;

/**
 * A `FlxSkewedSpriteGroup` that only allows specific members to be a specific type of `FlxSkewed`.
 * To use any kind of `FlxSkewed` use `FlxSkewedSpriteGroup`, which is an alias for
 * `FlxSkewedTypedSpriteGroup<FlxSkewed>`.
 */
class FlxSkewedTypedSpriteGroup<T:FlxSkewed> extends FlxSkewed
{
  /**
   ...
   */
  override public function clone():FlxSkewedTypedSpriteGroup<T>
  {
    var newGroup = new FlxSkewedTypedSpriteGroup<T>(x, y, maxSize);
  ...
  /**
   * Checks to see if a point in 2D world space overlaps any `FlxSkewed` object from this group.
   *
  ...
  /**
   * Checks to see if a point in 2D world space overlaps any of FlxSkewed object's current displayed pixels.
  ...
  /**
   * Adds a new `FlxSkewed` subclass to the group.
   *
  ...
  /**
   * Inserts a new `FlxSkewed` subclass to the group at the specified position.
   *
   ...
   *
   * @param   ObjectClass     The class type you want to recycle (e.g. `FlxSkewed`, `EvilRobot`, etc).
   ...
   *
   * @param   sprite  The `FlxSkewed` you want to remove.
  ...
  /**
   * Replaces an existing `FlxSkewed` with a new one.
   *
   ...
   * @param   Force         Force the object to be an `ObjectClass` and not a super class of `ObjectClass`.
   * @return  A `FlxSkewed` currently flagged as not existing.
   */
   ...
   *
   * @return  A `FlxSkewed` currently flagged as existing.
   */
   ...
   * This is handy for checking if everything's wiped out, or choosing a squad leader, etc.
   *
   * @return  A `FlxSkewed` currently flagged as not dead.
   */
   ...
   * This is handy for checking if everything's wiped out, or choosing a squad leader, etc.
   *
   * @return  A `FlxSkewed` currently flagged as dead.
   */
   ...
   *
   * @return  The number of `FlxSkewed`s flagged as not dead. Returns `-1` if group is empty.
   */
   ...
   *
   * @return  The number of `FlxSkewed`s flagged as dead. Returns `-1` if group is empty.
   */
   ...
   * @return  A `FlxSkewed` from the `members` list.
   */
   ...
   *
   * @param   ObjectClass   A class that objects will be checked against before Function is applied, ex: `FlxSkewed`.
   * @param   Function      A function that modifies one element at a time.
   * @param   Recurse       Whether or not to apply the function to members of subgroups as well.
   */
  ...
  /**
   * Remove all instances of `FlxSkewed` from the list.
  ...
  inline function xTransform(Sprite:FlxSkewed, X:Float)
    Sprite.x += X; // addition
  ...
  inline function alphaTransform(Sprite:FlxSkewed, Alpha:Float)
  {
  ...
  inline function flipYTransform(Sprite:FlxSkewed, FlipY:Bool)
    Sprite.flipY = FlipY;

  inline function movesTransform(Sprite:FlxSkewed, Moves:Bool)
    Sprite.moves = Moves;

  inline function pixelPerfectTransform(Sprite:FlxSkewed, PixelPerfect:Bool)
    Sprite.pixelPerfectRender = PixelPerfect;

  inline function gColorTransform(Sprite:FlxSkewed, Color:Int)
    Sprite.color = Color;

  inline function blendTransform(Sprite:FlxSkewed, Blend:BlendMode)
    Sprite.blend = Blend;

  inline function immovableTransform(Sprite:FlxSkewed, Immovable:Bool)
    Sprite.immovable = Immovable;

  inline function visibleTransform(Sprite:FlxSkewed, Visible:Bool)
    Sprite.visible = Visible;

  inline function activeTransform(Sprite:FlxSkewed, Active:Bool)
    Sprite.active = Active;

  inline function solidTransform(Sprite:FlxSkewed, Solid:Bool)
    Sprite.solid = Solid;

  inline function aliveTransform(Sprite:FlxSkewed, Alive:Bool)
    Sprite.alive = Alive;

  inline function existsTransform(Sprite:FlxSkewed, Exists:Bool)
    Sprite.exists = Exists;

  inline function cameraTransform(Sprite:FlxSkewed, Camera:FlxCamera)
    Sprite.camera = Camera;

  inline function camerasTransform(Sprite:FlxSkewed, Cameras:Array<FlxCamera>)
    Sprite.cameras = Cameras;

  inline function offsetTransform(Sprite:FlxSkewed, Offset:FlxPoint)
    Sprite.offset.copyFrom(Offset);

  inline function originTransform(Sprite:FlxSkewed, Origin:FlxPoint)
  ...
  inline function clipRectTransform(Sprite:FlxSkewed, ClipRect:FlxRect)
  {
   ...
   */
  override public function loadGraphicFromSprite(Sprite:FlxSprite):FlxSkewed
  {
    #if FLX_DEBUG
  ...
  inline function get__sprites():Array<FlxSkewed>
  {
