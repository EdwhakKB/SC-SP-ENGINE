...
class AlphaCharacter extends FlxSprite
{
  ...
  public static function loadAlphabetData(request:String = 'alphabet')
  {
    ...
    if (#if MODS_ALLOWED !FileSystem.exists(path) #else !Assets.exists(path, TEXT) #end) path = Paths.getPath('images/alphabet.json');
  ...
  public function setupAlphaCharacter(x:Float, y:Float, ?character:String = null, ?bold:Null<Bool> = null)
  {
    ...
    if (character != null)
    {
      ...
      var anim:String = alphaAnim + postfix #if mac + " instance 100" #end;
