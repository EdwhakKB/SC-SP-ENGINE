...
class GameJoltLogin extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
  ...
  override function create()
  {
    ...
    helpBox = new PsychUIButton(Math.floor(FlxG.width * 0.5), signInBox.y + 100, "GameJolt Token", function() {
      ...
      {
        Save.set('gjleaderboardToggle', !Save.get('gjleaderboardToggle'));
        Debug.logInfo('Is Score Board Enabled? ${Save.get('gjleaderboardToggle')}');
        Main.gjToastManager.createToast(GameJoltInfo.imagePath, "Score Submitting",
          "Score submitting is now " + (Save.get('gjleaderboardToggle') ? "Enabled" : "Disabled"), false);
        Save.flush();
      }
    ...
    logOutBox = new PsychUIButton(Math.floor(FlxG.width * 0.5), helpBox.y + 100, "Log Out & Close", function() {
      ...
      GameJoltAPI.connect();
      GameJoltAPI.authDaUser(Save.get('gjUser'), Save.get('gjToken'));
      Save.flush();
  ...
  override function update(elapsed:Float)
  {
    if (GameJoltAPI.getStatus())
    {
      helpBox.text.text = "Leaderboards:\n" + (Save.get('gjleaderboardToggle') ? "Enabled" : "Disabled");
      helpBox.color = (Save.get('gjleaderboardToggle') ? FlxColor.GREEN : FlxColor.RED);
    }
  ...
  function openLink(url:String)
    return scfunkin.utils.WindowUtil.openURL(url);
}
