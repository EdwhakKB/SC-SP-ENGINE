...
import scfunkin.backend.gamejolt.GameJoltGroup.GameJoltInfo;
import scfunkin.backend.gamejolt.GameJoltGroup.GameJoltLogin;
...
class GameJoltAPI // Connects to hxgamejolt.GameJolt
{
   ...
   */
  public static function addTrophy(id:Int)
  {
    if (userLogin)
    {
      GJApi.addTrophy(Save.get('gjUser'), Save.get('gjToken'), id,
        {
          onSucceed: function():Void {
  ...
  public static function pullTrophy(id:Int):Map<String, String>
  {
    ...
    GJApi.fetchTrophy(Save.get('gjUser'), Save.get('gjToken'), null, id,
      {
        onSucceed: function(data:Array<Tophy>):Void {
        },
        onFail: function(message:String):Void {
          Debug.logError(message);
        }
  ...
  public static function addScore(score:Int, tableID:Int, ?extraData:String)
  {
    ...
    if (GameJoltAPI.leaderboardToggle)
    {
      ...
      GJApi.addScore(Save.get('gjUser'), Save.get('gjToken'), "guest", score + "%20Points", score, formData, tableID, {
  ...
  {
    GJApi.openSessions(Save.get('gjUser'), Save.get('gjToken'),
      {
        onSucceed: function():Void {
  ...
  public inline static function pingSession()
    GJApi.pingSessions(Save.get('gjUser'), Save.get('gjToken'));

  /**
   * Closes the session, used for signing out
   */
  public inline static function closeSession()
    GJApi.closeSessions(Save.get('gjUser'), Save.get('gjToken'));
